...
function getTicketCost(d, {startDate,adults,children,parkDays,ticketStyle}){
  ...
  if(d.ticketModel==="starting"){
    if(d.key==="wdw" && ticketStyle==="best" && parkDays===4){const offer=d.offers.find(o=>o.title.includes("4-Park Magic"));if(offer && startDate<=offer.validThrough)return {total:offer.price*adults,confidence:"verified",message:"Using the published 4-Park Magic Ticket starting price for adults. Child pricing and tax still require a live check."}}
    return {total:d.adultOneDayFrom*adults*parkDays,confidence:"estimate",message:`Uses the published adult starting price of ${money(d.adultOneDayFrom,d.currency)} per day. Date, park, child pricing and tax can change the live total.`}
  }
